...
export default function CustomizedTables() {
  ...
  const addWalletTokenData = walletTokenData => () => {
    checkLocalTokenInfo(walletTokenData.contractAddress)
      ?
      removeLocalTokenInfo(walletTokenData.contractAddress)
      :
      storeLocalTokenInfo(walletTokenData.contractAddress, walletTokenData.name, walletTokenData.symbol, walletTokenData.value, walletTokenData.rate, walletTokenData.rateAmount)
    reloadComponent()
  }
  ...
  return (
    <div>
      <Grid container className={classes.tableHead}>
        ...
        <Grid item xs={6}>
          <div onClick={reloadComponent} className={classes.modalRight}>
            Load new tokens
          </div>
        </Grid>
      </Grid>
              ...
              walletOwnTokens.map((walletOwnToken, index) => (
                <StyledTableRow key={index}>
                  <StyledTableCell component="th" scope="row">
                    <Link
                      to={{
                        pathname: `/tokens/${walletOwnToken.contractAddress}`,
                      }}
                      onClick={dispatchTokenInfo(walletOwnToken.contractAddress)}
                      className={classes.linkToken}
                    >
                      {walletOwnToken.symbol}&nbsp;
                      <span className={'textSuccess'}>${walletOwnToken.rate}</span>
                      <br />
                      <span className={'textMuted'}>{walletOwnToken.name}</span>
                    </Link>
                  </StyledTableCell>
                  <StyledTableCell>
                  ...
                  <StyledTableCell>
                    {
                      checkLocalTokenInfo(walletOwnToken.contractAddress) == true ?
