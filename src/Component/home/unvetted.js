...
function UnvettedTable(props) {
  ...
  const addUnvettedData = unvettedData => () => {
    checkLocalTokenInfo(unvettedData.address)
      ?
      removeLocalTokenInfo(unvettedData.address)
      :
      storeLocalTokenInfo(unvettedData.address, unvettedData.name, unvettedData.symbol, unvettedData.value, unvettedData.rate, unvettedData.rateAmount)
    props.reloadData()
  }
  return (
    values.map((item, index) =>
      ...
      <Link
            ...
            onClick={() => dispatch({ type: 'SET_TOKENADDRESS', payload: item.address })}
...
export default function CustomizedTables() {
  ...
  useEffect(() => {
    unvettedValues(account, setUnvettedValues);
  }, []);
